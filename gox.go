...
func importQLNonGUIPackages() {
	...
	var defaultExports = map[string]interface{}{
		...
		"recsToMapStrMap":         tk.TableToMSSMap,
		"recsToMapStrMapX":        tk.RecordsToMapStringMap,    // 可以支持多种入参类型
		...
		"recsToMapStrOrderedMap":  tk.TableToMSSOrderedMap,     // 将多行行（第一行为标头字段行）的常见于SQL语句查询结果（[][]string格式或[][]interface{}格式）变为类似{"Value1": {"Field1": "Value1", "Field2": "Value2"}, "Value1a": {"Field1": "Value1a", "Field2": "Value2a"}}的map[string]*tk.OrderedMap格式，注意与recsToMapArrayMap的区别是返回结果中每个叶子项是个有序列表
		"recsToMapStrOrderedMapX": tk.RecordsToOrderedMapStringMap,
